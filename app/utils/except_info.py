...
def web_warning(PCAPS, host_ip):
    ...
    for ip, count in warn_ip.items():
        webwarn_list.append({'ip_port': ip, 'warn':u'HTTP brute force cracking', 'time': str(count), 'data':None})
    return webwarn_list
...
def ftp_warning(PCAPS, host_ip):
    ...
    warn_ip = {k:y for k,y in ip_count.items() if y>10}
    for ip, count in warn_ip.items():
        ftpwarn_list.append({'ip_port': ip, 'warn':u'FTP brute force cracking', 'time': str(count), 'data':None})
    return ftpwarn_list
...
def telnet_warning(PCAPS, host_ip):
    ...
    warn_ip = {k:y for k,y in ip_count.items() if y>10}
    for ip, count in warn_ip.items():
        telnetwarn_list.append({'ip_port': ip, 'warn':u'Telnet brute force cracking', 'time': str(count), 'data':None})
